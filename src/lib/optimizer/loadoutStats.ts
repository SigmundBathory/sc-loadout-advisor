  ...
  for (const componentId of Object.values(componentIds)) {
    ...
    switch (comp.type) {
      ...
      case "MissileRack":
        ...
        break;
      case "Shield":
        shield_hp += Math.min(s.hp || 0, 15000); // Cap at 15k (slightly above max size 2)
        shield_regen += Math.min(s.regen_rate || 0, 2500); // Cap at 2500 (slightly above max size 2)
        break;
      case "PowerPlant":
        power_output += s.output || 0;
        break;
      case "Cooler":
        cooling_rate += Math.min(s.cooling_rate || 0, 50); // Cap cooling (normal max ~30-40)
        break;
      case "QuantumDrive":
