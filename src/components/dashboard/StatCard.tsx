...
export default function StatCard({
  ...
  return (
    <div
      className="product-card product-card-hover group p-5 sm:p-6"
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        <div
          className={cn(
            "p-2.5 rounded-xl bg-muted/55 border border-border/40 group-hover:scale-110 transition-transform"
          )}
          style={{ color: glow }}
        >
          {icon}
        ...
        {subtitle && (
          <span className="text-[10px] uppercase tracking-wider text-muted-foreground bg-muted/55 border border-border/40 px-2 py-1 rounded-full">
            {subtitle}
          </span>
        )}
      </div>
      <div className="relative mt-4">
        <p className={cn("text-3xl sm:text-4xl font-bold tracking-tight font-heading", color)}>
